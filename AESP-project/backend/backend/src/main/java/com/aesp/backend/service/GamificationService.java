...
public class GamificationService {
    ...
    public void updateStreak(Long userId) {
        ...
        if (lastDate == null) {
            // Trường hợp: Lần đầu tiên học
            stats.resetStreak(); 
            System.out.println("User " + userId + ": Start first streak.");
        } 
        else if (lastDate.isEqual(today)) {
            // Trường hợp 3: Đã học hôm nay -> Không làm gì cả
            return; 
        } 
        else if (lastDate.isEqual(today.minusDays(1))) {
            // Trường hợp 1: Học liên tục (Hôm qua có học) -> Tăng Streak
            stats.incrementStreak();
            System.out.println("User " + userId + ": Streak incremented to " + stats.getCurrentStreak());
        } 
        else {
            // Trường hợp 2: Đứt quãng (Hôm kia trở về trước) -> Reset Streak
            stats.resetStreak();
            System.out.println("User " + userId + ": Streak broken. Reset to 1.");
        }
